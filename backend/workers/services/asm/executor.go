...
import (
	...
	"workers/config"
	aisubs "workers/tools/ai/subdomainprobe"
	amass "workers/tools/amass"
	bbot "workers/tools/bbot"
	cloudenum "workers/tools/cloudenum"
	"workers/tools/crtsh"
	dnsgen "workers/tools/dnsgen"
	"workers/tools/dnsx"
	emailleak "workers/tools/emailleak"
	gobuster "workers/tools/gobuster"
	"workers/tools/httpprobe"
	"workers/tools/httpx"
	katana "workers/tools/katana"
	naabu "workers/tools/naabu"
	nmap "workers/tools/nmap"
	"workers/tools/nuclei"
	reposcan "workers/tools/reposcan"
	saasdetect "workers/tools/saasdetect"
	sslscan "workers/tools/sslscan"
...
func getAssetName(ctx context.Context, assetID string) (string, error) {
	...
	assetName, err := AssetName(queryCtx, assetID)
	if err != nil {
...
func Run(ctx context.Context, task Task) (result Result) {
	...
	basePipeline, err := GetPipeline(ctx, task.JobID)
	if err != nil {
	...
	for i := range enhancedPipeline.Pipeline {
		...
		// Per-stage panic recovery (audit): a panic inside a single tool/parser must
		// NOT crash the worker or abort the whole scan. It is caught here, converted
		// to a stage error, and the pipeline continues to the next stage (the failed
		// stage is recorded as SKIPPED with the panic detail — see failure handling).
		func() {
			defer func() {
				if r := recover(); r != nil {
					stagePanic = true
					toolErr = fmt.Errorf("panic in tool %s: %v", enhancedPipeline.Pipeline[i].Tool, r)
					utils.Logger.Errorf("recovered panic in tool job=%s step=%d tool=%s: %v",
						task.JobID, i, enhancedPipeline.Pipeline[i].Tool, r)
				}
			}()
			...
			case "asset_diff_engine":
				// Asset change detection (current vs previous scan) is not yet
				// implemented. Report it honestly as SKIPPED (asset_diff_engine is an
				// optional tool) rather than emitting a fake "completed / no changes"
				// result that makes the Changes tab always look clean.
				utils.Logger.Infof("asset_diff_engine not implemented, skipping job=%s step=%d", task.JobID, i)
				toolErr = errors.New("asset_diff_engine not implemented (change detection pending)")
			...
			case "ip_tls_deep":
				ips := collectIPTargets(enhancedPipeline.Pipeline, i, enhancedPipeline.Pipeline[i].AssetID)
				// Determine known TLS ports from prior port-scan results (443 plus
				// any other discovered TLS-ish ports); default to 443 if none.
				tlsPorts := collectTLSPorts(enhancedPipeline.Pipeline, i, enhancedPipeline.Pipeline[i].AssetID)
				tlsResults := runTLSHandshake(ctx, ips, tlsPorts)
				output = map[string]interface{}{
			...
			case "ip_relationship_map":
				ips := collectIPTargets(enhancedPipeline.Pipeline, i, enhancedPipeline.Pipeline[i].AssetID)
				asnByIP := enrichmentByIP(enhancedPipeline.Pipeline, i, enhancedPipeline.Pipeline[i].AssetID)
				output = map[string]interface{}{
					"relationships": computeIPRelationships(ips, asnByIP),
				}

			case "ip_diff":
				ips := collectIPTargets(enhancedPipeline.Pipeline, i, enhancedPipeline.Pipeline[i].AssetID)
				currentPorts := collectPortResults(enhancedPipeline.Pipeline, i, enhancedPipeline.Pipeline[i].AssetID)
			...
			// -------------------------
			// Service ASM tools (LIGHT/NORMAL/DEEP)
			// -------------------------
			case "http_banner_check":
				targets, err := buildServiceSeedTargets(ctx, task.JobID, enhancedPipeline.Pipeline[i].AssetID)
				if err != nil {
					toolErr = err
					break
				}
				banners := runHTTPBannerCheck(ctx, targets)
				output = map[string]interface{}{
					"banners": banners,
					"count":   len(banners),
				}
				utils.Logger.Infof("http_banner_check completed job=%s step=%d targets=%d", task.JobID, i, len(targets))

			case "top_ports_services":
				targets, err := buildServiceSeedTargets(ctx, task.JobID, enhancedPipeline.Pipeline[i].AssetID)
				if err != nil {
					toolErr = err
					break
				...
				for _, t := range targets {
					hosts = append(hosts, hostOnly(t))
				}
				portResults, scanErr := naabu.RunTopPortsScan(ctx, hosts)
				if scanErr != nil {
					// Fall back to nmap top-ports (naabu binary may be absent).
					utils.Logger.Warnf("top_ports_services: naabu failed (%v), falling back to nmap", scanErr)
					ports := runNmapPortScan(ctx, hosts, 100, false, false)
					services := runServiceFingerprint(ctx, ports, enhancedPipeline.Intensity)
					output = map[string]interface{}{
						"ports":    ports,
						"services": services,
						"count":    len(ports),
				...
				ports := make([]map[string]interface{}, 0, len(portResults))
				for _, pr := range portResults {
				...
				utils.Logger.Infof("top_ports_services completed job=%s step=%d ports=%d services=%d", task.JobID, i, len(ports), len(services))

			case "deep_misconfig_analysis":
				targets, err := buildServiceSeedTargets(ctx, task.JobID, enhancedPipeline.Pipeline[i].AssetID)
				if err != nil {
					toolErr = err
					break
				}
				findings, scanErr := nuclei.Run(ctx, targets)
				if scanErr != nil {
					toolErr = scanErr
					utils.Logger.Errorf("deep_misconfig_analysis failed job=%s step=%d error=%v", task.JobID, i, scanErr)
					break
				}
				...
				utils.Logger.Infof("deep_misconfig_analysis completed job=%s step=%d findings=%d", task.JobID, i, len(findingList))
		...
		}() // end per-stage panic-recovery wrapper (opened above the switch)
		...
		if toolErr != nil {
			// A panic in a stage is contained (stagePanic) and recorded as
			// SKIPPED so one bad tool can't abort the whole scan — same handling
			// as an optional-tool failure.
			if isOptionalTool(enhancedPipeline.Pipeline[i].Tool) || stagePanic {
...
func saveEnhancedPipeline(ctx context.Context, pipeline *EnhancedPipeline) error {
	...
	if err := SavePipelineRaw(redisCtx, pipelineKey, string(data)); err != nil {
...
func fail(jobID string, err error) Result {
...
func isOptionalTool(tool string) bool {
	optionalTools := map[string]bool{
		...
		// service asset-type pipeline tools that depend on external binaries
		// (naabu/nmap/nuclei). http_banner_check has a native fallback so it is
		// intentionally NOT optional. top_ports_services / deep_misconfig_analysis
		// skip cleanly if their binaries are absent rather than failing the scan.
		"top_ports_services":      true,
		"deep_misconfig_analysis": true,
	}
