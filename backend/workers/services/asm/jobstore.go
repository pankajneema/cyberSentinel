...
func GetPipeline(ctx context.Context, jobID string) (*Pipeline, error) {
	...
	val, err := config.Get(redisCtx, pipelineKey(jobID))
	if err != nil {
...
func SavePipeline(ctx context.Context, jobID string, p *Pipeline) error {
	...
	if err := config.Set(redisCtx, pipelineKey(jobID), string(data), pipelineTTL); err != nil {
...
func SavePipelineRaw(ctx context.Context, key string, data string) error {
	...
	if err := config.Set(redisCtx, key, data, pipelineTTL); err != nil {
