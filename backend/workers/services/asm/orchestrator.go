...
func RegisterJob(job *Job) error {
	...
	raw, err := DiscoveryJSON(ctx, job.ID)
	if err != nil {
	...
	// signal the consumer to ACK instead of dead-letter.
	...
	if err := config.Set(redisCtx, "asm:pipeline:"+job.ID, pipelineJSON, 24*time.Hour); err != nil {
	...
	if err := SetDiscoveryRunning(ctx, job.ID); err != nil {
...
func publishScanEventSync(discoveryID, event, errMsg string) {
	...
	if err := config.Publish(ctx, "asm:worker:events", payload); err != nil {
...
// terminal result. It is the exported entry point the ASM consumer handler
// calls so that the queue ACK completes only after
...
func executeJob(jobID string) error {
	...
	ctx, cancel := NewTimeoutContext(config.Load().TaskTimeoutSec)
	defer cancel()
	...
	if !result.Success {
		if err := SetDiscoveryFailed(statusCtx, jobID); err != nil {
	...
	if err := SetDiscoveryCompleted(statusCtx, jobID); err != nil {
